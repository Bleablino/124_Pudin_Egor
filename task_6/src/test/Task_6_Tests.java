...
public class Task_6_Tests {
    ...
    public static void test(Task_6 task, String name) {
        ...
        String[] subtask_names = getSubtasks();
    ...
    private static boolean testTask1Functionality(StringBuilder protocol) {
        boolean[] results = new boolean[]{
            testObject1(new BattleUnit(5, 2, 10, 0, 0), 2, 10, new BattleUnit(4, 2, 10, 0, 0), 2),
            testObject1(new BattleUnit(5, -2, 10, 0, 0), -2, 10, new BattleUnit(4, 2, 10, 0, 0), 4),
            testObject1(new BattleUnit(5, 2, -10, 0, 0), 2, -10, new BattleUnit(4, 2, 10, 0, 0), 2),
            testObject1(new BattleUnit(5, 2, 10, 0, 0), 2, 10, new BattleUnit(1, 2, 10, 0, 0), 0),
            testObject1(new BattleUnit(5, 2, 10, 0, 0), 2, 10, new BattleUnit(4, 2, 10, 0, 0), 2)
        };
        boolean res = true;
        for(int i = 0; i < results.length; ++i) {
    ...
    private static boolean testObject1(
            BattleUnit battleUnit, int armor, int health, BattleUnit enemy, int validDamage
    ) {
        boolean r1 = battleUnit.getArmor() == armor
            && battleUnit.isAlive() == (health > 0)
            && battleUnit.getStrength() == 5
            && battleUnit.getX() == 0
            && battleUnit.getY() == 0
