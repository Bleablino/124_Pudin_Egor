public class Task_1 implements Task_1_base {
    ...
    public int subtask_3_if(double x, double y, double left_up_x, double left_up_y, double width, double height) {
        ...
        // ------------------------------------------------------------------------------------
        if(width == height)
            if((Math.abs(left_up_x + width - x) <= 0.000001 || Math.abs(x-left_up_x) <= 0.000001) || (Math.abs(left_up_y - y) <= 0.000001 || Math.abs(y - left_up_y - height) <= 0.000001))
                return 0;
    ...
    public int subtask_4_if(double x0, double y0, double k, double b) {
        ...
        // Допустимой погрешностью при сравнении переменных типа double считать 0.000001
        // ------------------------------------------------------------------------------------
        double y = Math.abs(k*x0+b);
        if (Math.abs(y-y0)<=0.000001)
            return 2;
        else if (y>y0)
            return 1;
        else
    ...
    public int subtask_7_if(double vx, double vy, double vz, double speed, double time, double wall) {
        ...
        // Допустимой погрешностью при сравнении переменных типа double считать 0.000001
        // ------------------------------------------------------------------------------------
        double way = Math.sqrt(wall*wall*Math.abs(vx) + wall*wall*Math.abs(vy) + wall*wall*Math.abs(vz));
        if(time > 0 && speed > 0)
            if((way/speed <= time && vx != 0) || Math.abs(way/speed - time) <= 0.000001 )
                return 1;
            else
                return 0;
        else
